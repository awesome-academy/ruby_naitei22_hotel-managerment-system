...
RSpec.describe Admin::AmenitiesController, type: :controller do
  ...
  let!(:amenity1) { create(:amenity, name: "Wifi", description: "High speed wifi") }
  let!(:amenity2) { create(:amenity, name: "Parking", description: "Free parking") }
  let!(:amenity3) { create(:amenity, name: "Pool", description: "Outdoor pool luxury") }
  ...
  describe "GET #index" do
    ...
    context "with query param name" do
      it "filters amenities by name" do
        get :index, params: { q: { name_cont: "Wifi" } }
  ...
  describe "POST #create" do
    context "with valid params" do
      let(:valid_params) { { amenity: attributes_for(:amenity, name: "Gym", description: "Fully equipped") } }
      ...
      it "assigns correct name" do
        ...
        expect(Amenity.last.name).to eq "Gym"
      end

      it "assigns correct description" do
        post :create, params: valid_params
        expect(Amenity.last.description).to eq "Fully equipped"
      end
    ...
    context "with invalid params" do
      let(:invalid_params) { { amenity: { name: "", description: "Desc" } } }
    ...
    context "with unauthorized params" do
      let(:unauthorized_params) { { amenity: { name: "Spa", description: "Spa desc", hacked: "xxx" } } }
  ...
  describe "PATCH #update" do
    let(:new_name) { { name: "Updated Wifi" } }
    let(:new_description) { { description: "Updated high speed" } }

    context "with valid params" do
      it "updates the name" do
        patch :update, params: { id: amenity1.id, amenity: new_name }
        amenity1.reload
        expect(amenity1.name).to eq "Updated Wifi"
      end

      it "updates the description" do
        patch :update, params: { id: amenity1.id, amenity: new_description }
        amenity1.reload
        expect(amenity1.description).to eq "Updated high speed"
      end

      it "sets success flash" do
        patch :update, params: { id: amenity1.id, amenity: { name: "New", description: "New desc" } }
    ...
    context "when amenity does not exist" do
      before { patch :update, params: { id: -1, amenity: { name: "X" } } }
    ...
    context "with unauthorized params" do
      let(:unauthorized_params) { { id: amenity1.id, amenity: { name: "Allowed", description: "Allowed desc", hacked: "xxx" } } }

      it "updates the name" do
        patch :update, params: unauthorized_params
        amenity1.reload
        expect(amenity1.name).to eq "Allowed"
      end

      it "updates the description" do
        patch :update, params: unauthorized_params
        amenity1.reload
        expect(amenity1.description).to eq "Allowed desc"
      end
